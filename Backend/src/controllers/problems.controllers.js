...
const updateProblem = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        if (!updateProblem) {
            return next(new ApiError(500, "Problem is not able to update in the database."))
        }

        return res.status(200).json(new ApiResponse(200, updateProblem, "Problem is updated successfully."))

    } catch (error) {
        return next(new ApiError(500,error,"Error while Updating the problem."))
    }
...
const deleteProblem = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        if(!isProblemExsit){
            return next(new ApiError(404,"Problem is Exsit."))
        }
        ...
        if(!deleteProblem){
            return next(new ApiError(500,"Problem is not delete from the database."))
        }
        return res.status(200).json(new ApiResponse(200,"Delete the problem is Sucessfully."));
    } catch (error) {   
        return next(new ApiError(500,error,"Error while delete the problem."))
    }
});
